                ...
                for i in range(MAX_MODEL_COUNT):
                    with FormRow(variant="compact"):
                        ...
                        with gr.Column() as col:
                            weight = gr.Slider(label=f"Weight {i+1}", value=1.0, minimum=-1.0, maximum=2.0, step=0.05, visible=False)
                            weight_unet = gr.Slider(
                                label=f"UNet Weight {i+1}", value=1.0, minimum=-1.0, maximum=2.0, step=0.05, visible=True
                            )
                            weight_tenc = gr.Slider(
                                label=f"TEnc Weight {i+1}", value=0.0, minimum=-1.0, maximum=2.0, step=0.05, visible=True
                            )
